<template>
  ...
  >
    ...
    <b-col cols="12">
      <template>
        <b-card-code style="background-color:rgba(33,33,33, 0.95);">
          <b-row>
            <b-col
              md="7"
              sm="12"
            ...
            >
              ...
              <b-button
                ...
                @click.prevent="openDiagramModal(`https://drr.enixma.net/semcontrol-detail1?wid=${wid}&type=${type}&imei=${imei}`, 'Diagram')"
              >
                Diagram
              </b-button>
              ...
              <b-button
                ...
                class="btn-tour-skip mr-1 diagram-button"
                style="background-color: #ff8c00; border-color: #ff8c00;"
                @click.prevent="openDiagramModal(`https://drr.enixma.net/SEMNB-iotCab3P?wid=${wid}&type=${type}&imei=${imei}`, 'Diagram')"
              >
                Diagram
              </b-button>

              <b-button
                v-if="phaseType === '1p_lora'"
                size="sm"
                variant="warning"
                class="btn-tour-skip mr-1 diagram-button"
                style="background-color: #ff8c00; border-color: #ff8c00;"
                @click.prevent="openDiagramModal(`https://drr.enixma.net/nbiot-lora?wid=${wid}&type=${type}&imei=${imei}`, 'Diagram')"
              >
                Diagram
              </b-button>
            </b-col>
...
export default {
  ...
  mounted() {
    // กำหนดค่า imei จาก props
    if (this.switch) {
      this.switch.imei = this.imei
    }
    ...
    // กำหนดค่า device จาก props
    this.device = this.phaseType
  ...
  methods: {
    ...
    openDiagramModal(url, title) {
      console.log('Opening diagram in new window:', url, 'with title:', title)
      // Create a new window or tab with the URL
      const newWindow = window.open(url, '_blank')
      newWindow.focus()

      // Emit an event to notify parent component
      this.$emit('diagram-opened', { url, title })
    },
    checkDiagram() {
      axios
        .post('/NB-IoTLogDiagram', { imei: this.imei, wid: this.wid })
        .then(response => {
    ...
    getTableNBIoTLog() {
      let url = `/api-go/logs4g?imei=${this.imei}`
      ...
      // เพิ่มพารามิเตอร์ phase_type ถ้ามีการส่งมา แปลงค่าให้ตรงกับที่ API ต้องการ
      if (this.phaseType) {
        // แปลงค่า phaseType เป็นตัวเลขที่ API ต้องการ
        let phaseValue = '1' // ค่าเริ่มต้นเป็น 1

        if (this.phaseType === '3p') {
          phaseValue = '3'
        } else if (this.phaseType === '1p' || this.phaseType === 'single' || this.phaseType === '1p_lora') {
          phaseValue = '1'
        }

        url += `&phase_type=${phaseValue}`
        console.log(`Converting phaseType ${this.phaseType} to API parameter phase_type=${phaseValue}`)
      }
    ...
    getTableNBIoTLogDebug() {
      axios
        .post('/tableNB-IoTLog-4G-Debug', { imei: this.imei, wid: this.wid, phaseType: this.phaseType })
        .then(response => {
          this.items = response.data
          this.totalRows = response.data.length
        })
    ...
    getTableNBIoTLogStatuus() {
      axios
        .post('/tableNB-Status-Electic', { imei: this.imei, wid: this.wid })
        .then(response => {
      ...
      if (this.bool) {
        this.$swal({
          ...
          if (result.value) {
            this.bool = !this.bool
            try {
              await axios.post('/swithTFM-4G', { imei: this.imei, drv1_cmd: true })
              this.$swal({
                icon: 'success',
        ...
        this.$swal({
          ...
          if (result.value) {
            this.bool = !this.bool
            try {
              await axios.post('/swithTFM-4G', { imei: this.imei, drv1_cmd: false })
              this.$swal({
                icon: 'success',
    ...
    checkNBIoTLog() {
      axios
        .post('/checkNB-IoTLog-Electic', { imei: this.imei })
        .then(response => {
          ...
          if (response.data && response.data.length > 0) {
            ...
            // Use the props values if available
            // this.wid = response.data[0].wid
            // this.phase_type = response.data[0].phase_type
          } else {
            console.warn('No data returned from checkNB-IoTLog-Electic API')
          }
        })
        .catch(error => {
          console.error('Error in checkNBIoTLog:', error)
        })
    },
    ...
    checkSwith() {
      axios
        .post('/checkSwith-4G-Electic', { imei: this.imei })
        .then(response => {
          // ตรวจสอบว่า response.data มีข้อมูลหรือไม่
          if (response.data && response.data.length > 0) {
            ...
            console.warn('No data returned from checkSwith-4G API')
          }
        })
        .catch(error => {
          console.error('Error in checkSwith:', error)
        })
    },
  },
}
  ...
  .diagram-fullscreen-modal .modal-dialog {
    max-width: 100%;
    margin: 0;
    height: 100%;
  }

  .diagram-fullscreen-modal .modal-header {
