    ...
    <b-card-code style="background-color:rgba(255,255,255, 0.9);">
      ...
      <div id="Report">
          ...
          <b-col>
            <div style="text-align: center; padding-top: 15px">
              <span style="font-size: 20px; color: #000000">
                รายงานการแสดงค่าการทำงานของตู้ควบคุม (รายวัน)<br>
        ...
        <b-col lg="12">
          <template>
            <div>
              <span style="font-size:20px; color: #000000;">กราฟแสดงค่าการทำงานของตู้ควบคุม (Amp รายวัน)</span>
              <app-echart-line
                :option-data="option"
...
export default {
  ...
  methods: {
    graphLine() {
      axios.post('/reportGraphSEMControl3PDay-Electic', this.date)
        .then(response => {
          this.xAxisData = []
    ...
    getHistory(date1) {
      this.date = date1
      axios
        .post('/reportSEMControl3PDay-Electic', date1)
        .then(response => {
