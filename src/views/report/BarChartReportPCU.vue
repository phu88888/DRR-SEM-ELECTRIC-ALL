    ...
    <b-row>
      ...
      >
        <b-col
          ...
          lg="12"
        >
          <span style="font-size:20px; color: #000;">ปริมาณจราจร (PCU)</span>
        </b-col>
        <div>
          <br>
          <app-echart-bar
...
import AppEchartBar from './AppEchartBarPCU.vue'
      ...
      optionBar: {
        tooltip: {
          ...
          formatter(d) {
            ...
            result += `(${formatNumber(totalValue)} PCU)<br/></span>`

            d.forEach(item => {
              result += `${item.marker}${item.seriesName} ${formatNumber(item.value)} PCU<br />`
            })
      ...
      axios
        ...
        .then(response => {
          ...
          this.xAxisData.forEach(value => {
            ...
            this.optionBar.series[0].data.push(value.bike_pcu)
            this.optionBar.series[1].data.push(value.car_pcu)
            this.optionBar.series[2].data.push(value.pickup_pcu)
            this.optionBar.series[3].data.push(value.taxi_pcu)
            this.optionBar.series[4].data.push(value.bus_pcu)
            this.optionBar.series[5].data.push(value.truck_pcu)
            this.optionBar.series[6].data.push(value.trailer_pcu)
          })
        })
        .catch(error => {
          console.log(error)
